...
import { ERC20MockABI, ERC20MockBytecode } from "./abis/ERC20Mock";
import {
  createWalletClient,
  defineChain,
  encodeAbiParameters,
...
import { QuoterMockABI, QuoterMockBytecode } from "./abis/QuoterMock";
import {
  Address,
  ExtensionBuilder,
  Interaction,
...
import { RosettaABI, RosettaBytecode } from "./abis/Rosetta";
...
export async function interactiveDemo() {
  ...
  `);
  console.log(`
    Welcome to the Rosetta interactive demo!
    Here, you will build the order interactively and watch them being executed using the Rosetta extension.
  ...
  let hash: SendTransactionReturnType;
  ...
  while (true) {
    ...
    let value = prompt(`
    Enter the amount of WBTC to mint for Alice (not scaled):`);

    if (!value) {
      throw new Error("No value provided");
    }

    hash = await wbtc.write.mint([alice.address, parseUnits(value, 8)], {
      account: admin,
    });
    ...
    value = prompt(`
    Enter the amount of USDC to mint for Bob (not scaled):`);

    if (!value) {
      throw new Error("No value provided");
    }

    hash = await usdc.write.mint([bob.address, parseUnits(value, 6)], {
      account: admin,
    });
    await client.waitForTransactionReceipt({ hash });
    ...
    value = prompt(`
    Enter the amount of EURC to mint for Bob (not scaled):`);

    if (!value) {
      throw new Error("No value provided");
    }

    hash = await eurc.write.mint([bob.address, parseUnits(value, 6)], {
      account: admin,
    });
    await client.waitForTransactionReceipt({ hash });
    ...
    value = prompt(`
    Enter the amount of USDT to mint for Bob (not scaled):`);

    if (!value) {
      throw new Error("No value provided");
    }

    hash = await usdt.write.mint([bob.address, parseUnits(value, 6)], {
      account: admin,
    });
    await client.waitForTransactionReceipt({ hash });
  ...
  `);
    ...
    prompt(`
    Alice and Bob's balance have been filled, press enter to continue..`);

    console.log(`
    We will now deploy Rosetta and set up the quotes.
  `);

    value = prompt(`
    Quote: EURC <-> WBTC
    Please enter the amount of X EURC for Y WBTC (in the format X-Y):`);
    ...
    if (value.split("-").length !== 2) {
      throw new Error("Invalid format, expected X-Y");
    }
    ...
    hash = await quoter.write.setQuote(
      [
        tokens["EURC"]!,
        tokens["WBTC"]!,
        parseUnits(value.split("-")[0]!, 6),
        parseUnits(value.split("-")[1]!, 8),
      ],
      {
        account: admin,
      },
    ...
    value = prompt(`
    Quote: USDT <-> WBTC
    Please enter the amount of X USDT for Y WBTC (in the format X-Y):`);
    ...
    while (true) {
      value = prompt(`
        Do you want to add another quote? (y/n)`);
      ...
      value = prompt(`
        Please enter the tokens you want to quote in the format X-Y (X is the token to sell, Y is the token to buy - eg. WBTC-USDT):`);
      ...
      const token0 = value.split("-")[0]!;
      const token1 = value.split("-")[1]!;
      ...
      value = prompt(`
        Please enter the quote in the format X-Y (X is the amount to sell, Y is the quote - eg. 0.01-1000):`);
      ...
      hash = await quoter.write.setQuote(
        [
          tokens[token0],
          tokens[token1]!,
          parseUnits(value.split("-")[0]!, token0 === "WBTC" ? 8 : 6),
          parseUnits(value.split("-")[1]!, token1 === "WBTC" ? 8 : 6),
        ],
        {
          account: admin,
        },
    ...
    value = prompt(`
    Please enter the amount of WBTC that Alice will sell (not scaled):`);

    if (!value) {
      throw new Error("No value provided");
    }

    const makingAmount = parseUnits(value, 8);

    value = prompt(`
    Please enter the amount of USDC that Alice wants for her WBTC (not scaled):`);

    if (!value) {
      throw new Error("No value provided");
    }

    const takingAmount = parseUnits(value, 6);
    ...
    const order = new LimitOrder(
      {
        ...
        makingAmount: makingAmount, // X WBTC
        takingAmount: takingAmount, // Y USDC
    ...
    Order built and signed, Bob can now fill it.
      - Salt:    ${order.salt}
      - Nonce:   ${order.makerTraits.nonceOrEpoch()}
  `);
  ...
  `);
    ...
    value = prompt(`
    Interactive demo completed, do you want to run it again? (y/n)`);
...
export async function oneTimeDemo() {
  console.log("----------".repeat(12));
  console.log(`
